...
def main():
    results_dir = "e:/nlp-for-disaster/exp3th/results"
    ...
    for model_name in models:
        ...
        for temp in temperatures:
            ...
            y_pred_info = sub_df['final_predicted_info']
            ...
            y_pred_human = sub_df['final_predicted_category']
            ...
            plt.title(f"Confusion Matrix (Category) - {model_name} Temp {temp} (Exp 3TH)")
    ...
    print(f"\nSaved Exp 3TH comparison metrics to {metrics_csv}")
    print(metrics_df)
    
    # Generate chart
    chart_path = os.path.join(results_dir, "th_model_comparison_chart.png")
    generate_chart(metrics_csv, chart_path, "Thai 2-Agent Sequential (Exp 3TH) F1 Comparison")
    print(f"Generated chart at {chart_path}")
    
    # Generate Exp 3 vs Other Comparison
    exp1_metrics_csv = "e:/nlp-for-disaster/exp1th/results/th_model_comparison_metrics.csv"
    exp2_metrics_csv = "e:/nlp-for-disaster/exp2th/results/th_model_comparison_metrics.csv"
    
    if os.path.exists(exp1_metrics_csv) and os.path.exists(exp2_metrics_csv) and not metrics_df.empty:
        print("Generating th_exp3_vs_other_comparison.csv...")
        exp1_df = pd.read_csv(exp1_metrics_csv)
        exp2_df = pd.read_csv(exp2_metrics_csv)
        
        # Merge exp1 and exp2 first
        merged_1_2 = pd.merge(
            exp1_df, exp2_df,
        ...
        final_merged = pd.merge(
            merged_1_2, metrics_df,
            on=['model', 'temperature']
        )
        final_merged = final_merged.rename(columns={
            'text_informativeness_f1': 'text_informativeness_f1_exp3th',
            'text_category_f1': 'text_category_f1_exp3th',
            'avg_latency_seconds': 'avg_latency_seconds_exp3th',
            'total_tokens_used': 'total_tokens_used_exp3th'
        })
        
        comparison_csv = os.path.join(results_dir, "th_exp3_vs_other_comparison.csv")
        final_merged.to_csv(comparison_csv, index=False)
        print(f"Saved comparison to {comparison_csv}")
    else:
        print("Exp 1TH or Exp 2TH metrics not found. Skipping cross-architecture comparison.")
