...
| **Exp 1F (Few-Shot)** | Flat — 1 Step, 1 Layer | V3 (Few-Shot & Hierarchy) | เพิ่มตัวอย่าง Few-shot ที่สะท้อนเกณฑ์การตรวจหาและขอบเขตที่สอดคล้องกับชุดข้อมูลจริง |
...
| **Exp 2F (Few-Shot)** | Joint — 2 Step, 1 Layer | V3 (Few-Shot & Hierarchy) | เพิ่มตัวอย่าง Few-shot 6 ข้อแบบเดียวกับ 1F โดยจัดรูปแบบเอาต์พุตให้ตรงกับ Schema สองเลเยอร์ |
...
| **Exp 3F (Few-Shot)** | Seq — 2 Agent Layer | V3 (Few-Shot & Hierarchy) | เพิ่มตัวอย่าง Few-shot 4 ข้อสำหรับ Agent 1 พร้อมเพิ่ม Critical Decision Hierarchy และ Aggressive Bias Rule |
...
* **ผลกระทบเชิงระบบ**: ส่งผลดีให้โมเดล **gemma-4** ในระบบ Sequential (Exp 3F) ขยับขีดความแม่นยำขึ้นจาก `0.5854` (V2) ไปเป็น **`0.6430` (V3) (+5.76%)** ซึ่งแสดงให้เห็นว่าการจัดการลำดับขั้นทางตรรกะ [4] สามารถลดผลกระทบของการส่งผ่านความผิดพลาดสะสมได้อย่างมีนัยสำคัญ
...
* **บทวิเคราะห์ผลลัพธ์เปรียบเทียบเชิงลึก**:
  1. **ความโดดเด่นในงานจำแนกความเกี่ยวข้อง (Informativeness F1)**: ระบบ Zero-shot MoE LLMs ของเรา (โดยเฉพาะ Typhoon-v2.5 ใน Exp 2E ที่ทำคะแนนได้ **`0.9070`** และ DeepSeek-v4-flash ใน Exp 2E (V3) ที่ทำได้ **`0.8971`**) สามารถเอาชนะโมเดลที่ผ่านการ Fine-tuned อย่าง BERT-base ที่ทำได้ดีที่สุด **`0.8420`** ไปอย่างมีนัยสำคัญ สิ่งนี้ยืนยันว่าการประมวลผลด้วยโมเดล MoE ขนาดใหญ่ที่มีฐานความรู้รอบตัวกว้างขวาง [3] สามารถคัดกรองขยะหรือสกัดว่าข่าวใดเกี่ยวข้องกับภัยพิบัติได้ดีกว่าโมเดลขนาดเล็กที่เทรนแบบเจาะจง
  2. **ความท้าทายในงานจำแนกหมวดหมู่กู้ภัยย่อย (Category F1)**: คะแนนสูงสุดของ MoE LLMs ในงานนี้อยู่ที่ **`0.6493`** (Typhoon-v2.5) ตามด้วย **`0.6432`** (DeepSeek-v4-flash) และ **`0.6430`** (Gemma-4) ซึ่งยังคงตามหลัง BERT-base แบบ Supervised (`0.7686`) อยู่ประมาณ **-11.9%** ปรากฏการณ์นี้เกิดขึ้นเนื่องจากสัญวิทยาและเกณฑ์การจำแนกหมวดหมู่ช่วยเหลือ (เช่น Affected Individuals vs. Rescue/Volunteering) ใน CrisisMMD มีสไตล์การระบุเฉพาะตัวและมีความคิดเห็นของมนุษย์เข้ามาเกี่ยวข้องสูง (Subjective Annotation Criteria) [2, 10] ส่งผลให้โมเดลที่รันแบบ Zero-shot/Few-shot โดยไม่มีการเปลี่ยนค่าน้ำหนักพารามิเตอร์ภายใน (No Parameter Updates) พลาดโอกาสในการจำขอบเขตการตัดสินใจ (Decision Boundaries) เฉพาะตัวของชุดข้อมูลนี้ [7]
...
| **Seq** (Exp 3, 3E, 3F) | 0.0 | 0.5780 | 0.5854 | 0.6360 | **0.6380** | **+6.00%** |
...
### 6.4 ตารางสรุปผลคะแนนเฉลี่ยการทดลองทั้งหมด 17 การทดลอง (Overall 17 Experiments Average Summary Table)

ตารางเปรียบเทียบค่าเฉลี่ยคะแนนประสิทธิภาพจำแนกหมวดหมู่ภัยพิบัติย่อย (Category F1-Score) ข้ามระดับอุณหภูมิ (T=0.0 - 0.3) ของโมเดล MoE ทั้ง 3 รุ่น ตลอดทั้ง 17 การทดลองในโครงการ:

| การทดลอง | Typhoon | Gemma | DeepSeek | หมายเหตุ |
| :--- | :---: | :---: | :---: | :--- |
| **Exp 1** | 0.5688 | 0.5373 | 0.5587 | Flat V1 (EN) - ดั้งเดิม (มี Bias) |
| **Exp 1E** | 0.6180 | 0.6251 | 0.6038 | Flat V2 (EN) - ปรับปรุงคำสั่งเชิงบวก |
| **Exp 1F** | 0.5900 | 0.6302 | 0.6086 | Flat V3 (EN) - Few-Shot & Hierarchy |
| **Exp 1E-COT** | 0.6097 | 0.6214 | 0.5937 | Flat COT (EN) - Zero-Shot + COT |
| **Exp 2** | 0.4091 | 0.4195 | 0.4789 | Joint V1 (EN) - ดั้งเดิม (มี Bias) |
| **Exp 2E** | 0.6442 | 0.6014 | 0.6065 | Joint V2 (EN) - ปรับปรุงคำสั่งเชิงบวก |
| **Exp 2F** | 0.6241 | 0.6123 | 0.6191 | Joint V3 (EN) - Few-Shot & Hierarchy |
| **Exp 2E-COT** | 0.6165 | 0.6085 | 0.6235 | Joint COT (EN) - Zero-Shot + COT |
| **Exp 3** | 0.5622 | 0.5768 | 0.5515 | Seq V1 (EN) - ดั้งเดิม (มี Bias) |
| **Exp 3E** | 0.5659 | 0.5811 | 0.5702 | Seq V2 (EN) - ปรับปรุงคำสั่งเชิงบวก |
| **Exp 3F** | 0.5984 | 0.6409 | 0.6034 | Seq V3 (EN) - Few-Shot & Hierarchy |
| **Exp 3E-COT** | 0.5901 | 0.6401 | 0.6062 | Seq COT (EN) - Zero-Shot + COT |
| **Exp 1TH** | 0.6855 | 0.7541 | 0.7494 | Flat (TH) - ภาษาไทยขั้นตอนเดียว |
| **Exp 2TH** | 0.6954 | 0.7151 | 0.7256 | Joint (TH) - ภาษาไทยสองเลเยอร์ร่วม |
| **Exp 3TH** | 0.7377 | 0.7111 | 0.7558 | Seq (TH) - ภาษาไทยสองเอเจนต์ |
| **Exp 4** | N/A | N/A | N/A | Thai NER & Severity (ไม่มีการคำนวณคลาสจำแนกเปรียบเทียบ) |
| **Exp 4TH** | N/A | N/A | N/A | Thai NER & Severity Localization (ไม่มีผลลัพธ์ F1-score ในไฟล์) |

*\*หมายเหตุ: ค่าเฉลี่ยคำนวณจากสูตร: (F1@0.0 + F1@0.1 + F1@0.2 + F1@0.3) / 4*
...
2. **สำหรับสถาปัตยกรรมที่มีลำดับการทำงานซับซ้อน (Best Sequential Architecture)**:
   * **การกำหนดค่า**: ใช้โมเดล **`gemma-4`** บนสถาปัตยกรรม **Sequential (Exp 3F)** กำหนดระดับอุณหภูมิ **`Temp = 0.1`** และใช้ Prompt เวอร์ชัน **V3**
