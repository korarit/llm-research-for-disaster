...
*   **ชื่อและชื่อเล่นภาษาไทย (Name & Nickname Randomization)**:
    *   **ชื่อจริงและเพศ**: สุ่มรายชื่อตามเพศจากไฟล์ข้อมูลจริงในโฟลเดอร์ `dataset/thai_name/` (`female_names_th.txt` และ `male_names_th.txt` สำหรับชื่อต้น และ `family_names_th.txt` สำหรับนามสกุล)
    *   **ชื่อเล่นภาษาไทย**: สุ่มรายชื่อเล่นตามเพศจากไฟล์ `nickname_female_th.txt` และ `nickname_male_th.txt` (จำนวนไฟล์ละ 500 รายชื่อ)
    *   **กรณีมีคำนำหน้าแบบแยกเพศ (Gender-Aware Prefix)**: สุ่มคำนำหน้าเครือญาติหรือคำสุภาพตามเพศจริงของรายชื่อ เช่น เพศหญิงจะสุ่มได้กลุ่ม `นาง`, `นางสาว`, `คุณ`, `พี่`, `น้อง`, `ป้า`, `ยาย`, `เจ๊` ส่วนเพศชายจะสุ่มได้กลุ่ม `นาย`, `คุณ`, `พี่`, `น้อง`, `ลุง`, `ตา`, `เฮีย`
    *   **กรณีใช้เฉพาะชื่อเล่น (Use Only Nickname)**: สุ่มโอกาส 20% ที่ผู้ติดต่อจะใช้เฉพาะชื่อเล่นในการโพสต์แสดงตัวตน (เช่น "เจ๊ส้ม" หรือ "เบียร์" แทนชื่อเต็มปิยะ) และบันทึกเฉลยไว้ในฟิลด์ nickname
    *   **รูปแบบของชื่อเต็มและนามสกุล**: สุ่มการปรากฏของคำนำหน้าและนามสกุลในอัตราส่วน 50% เพื่อจำลองความหลากหลายในการพิมพ์จริง
*   **กรณีผู้ประสบภัยเป็นผู้แจ้งเหตุเอง (Same Person Scenario)**:
    *   สุ่มโอกาส 40% ที่ **ผู้แจ้งเหตุ (Reporter)** และ **ผู้ประสบภัย (Victim)** จะเป็นบุคคลเดียวกัน (มีชื่อจริง ชื่อเล่น เพศ และเบอร์โทรศัพท์เดียวกันทั้งหมด)
    *   สุ่มโอกาส 60% ที่จะเป็นคนละบุคคลกัน (เช่น ญาติหรือผู้พบเห็นแจ้งช่วยเหลือแทน)
    *   ข้อความที่โมเดลสังเคราะห์ขึ้นจะใช้สรรพนามและมุมมองที่สอดคล้อง (First-person vs. Third-person) และระบุชื่อ/เบอร์ติดต่อเพียงชุดเดียวหากเป็นคนเดียวกัน
*   **หมายเลขโทรศัพท์มือถือไทย**: สุ่มรูปแบบตัวเลข (เช่น `0812345678`, `092-345-6789`, `063 456 7890`)
*   **กรณีการสุ่มโทรศัพท์ (Phone Scenarios)**:
    *   **กรณีคนละบุคคล**: สุ่มรูปแบบ A (มีเบอร์คู่), B (มีเฉพาะเบอร์ผู้ประสบภัย), C (มีเฉพาะเบอร์ผู้แจ้ง), D (ไม่มีเบอร์เลย)
    *   **กรณีบุคคลเดียวกัน**: สุ่มว่ามีเบอร์ (80%) หรือไม่มีเบอร์ (20%) โดยหากมีเบอร์ ทั้งคู่จะแชร์เบอร์เดียวกัน เสมือนมีเบอร์เดียวปรากฏในโพสต์จริง
...
| `gt_victim_gender`   | เพศของผู้ประสบภัยเฉลยจริง                            | `female` / `male`                                                                                                                                                                                                                                              |
| `gt_victim_nickname` | ชื่อเล่นของผู้ประสบภัยเฉลยจริง (ถ้ามี)                  | `ป้าดา`                                                                                                                                                                                                                                                        |
| `gt_reporter_name`   | ชื่อผู้แจ้งเหตุเฉลยจริง                               | `วิชิต`                                                                                                                                                                                                                                                        |
| `gt_reporter_phone`  | เบอร์โทรผู้แจ้งเหตุเฉลยจริง                           | `092-111-2222`                                                                                                                                                                                                                                                 |
| `gt_reporter_gender` | เพศของผู้แจ้งเหตุเฉลยจริง                             | `male` / `female`                                                                                                                                                                                                                                              |
| `gt_reporter_nickname` | ชื่อเล่นของผู้แจ้งเหตุเฉลยจริง (ถ้ามี)                 | `เบียร์`                                                                                                                                                                                                                                                       |
| `gt_victims_json`    | ข้อมูลผู้ประสบภัยรายบุคคลเชิงลึกเฉลยจริง (JSON String) | `[{"name": "ป้าสมศรี", "age": null, "age_group": "adult", "age_disclosure": "indirect", "gender": "female", "triage_color": "YELLOW", "symptoms_literal": "แขนขาอ่อนแรงครึ่งซีกกะทันหัน ปากเบี้ยว"}, ...]`                                                        |
