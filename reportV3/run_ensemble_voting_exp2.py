...
def evaluate_ensemble_for_exp(exp_dir, exp_name):
    ...
    for temp in temperatures:
        ...
        for idx in range(n_rows):
            ...
            cat_votes = [df_temp[m].loc[idx, cat_col] for m in models]
        ...
        individual_metrics = {}
        for m in models:
            info_f1 = f1_score(y_true_info, df_temp[m][info_col], pos_label="informative", average="binary")
            cat_f1 = f1_score(y_true_human, df_temp[m][cat_col], average="weighted")
...
def main():
    ...
    print("Evaluating voting ensemble for Exp 2...")
    df_2 = evaluate_ensemble_for_exp(os.path.join(base_dir, "exp2"), "exp2")
    
    print("Evaluating voting ensemble for Exp 2E...")
    df_2e = evaluate_ensemble_for_exp(os.path.join(base_dir, "exp2E"), "exp2E")
    
    print("Evaluating voting ensemble for Exp 2F...")
    df_2f = evaluate_ensemble_for_exp(os.path.join(base_dir, "exp2F"), "exp2F")
    
    table_2 = format_exp_markdown(df_2, "Exp 2: Original Two-Layer Joint")
    table_2e = format_exp_markdown(df_2e, "Exp 2E: Optimized Two-Layer Joint")
    table_2f = format_exp_markdown(df_2f, "Exp 2F: Optimized Two-Layer Joint Few-Shot")
    
    output_path = os.path.join(base_dir, "reportV3", "raw_tables_exp2.txt")
